...
void ClauseMinimizer::minimizeLearnedClauses() {
    ...
    for (size_t i = 0; i < db.clauses.size(); i++) {
        if (!db.clauses[i] || !db.clauses[i]->is_learned) continue;
...
void ClauseMinimizer::binaryResolution(Clause& clause) {
    ...
    for (size_t i = 0; i < db.clauses.size(); i++) {
        if (!db.clauses[i] || db.clauses[i]->size() != 2) continue;
...
void ClauseMinimizer::vivification(Clause& clause) {
    ...
    for (int lit : clause) {
        int var = std::abs(lit);
        int level = var_to_trail.find(var) != var_to_trail.end() ? 
                  decision_levels[var] : std::numeric_limits<int>::max();
