...
ClauseDatabase::ClauseDatabase(size_t num_vars, bool debug)
    : clause_activity_inc(1),
      ...
      active_learned(0),
      deleted_learned(0),
      max_learnt_clauses(num_vars * 2), // Start with 2x variables as max
      clause_deletion_threshold(3.0),   // Initial LBD threshold
      allow_clause_deletion(true),
      debug_output(debug) {
    
    // Initialize watches array (2 * num_vars + 1 to handle both polarities, 1-indexed)
...
ClauseID ClauseDatabase::addLearnedClause(const Clause& clause, int lbd) {
    ...
    if (allow_clause_deletion && active_learned > max_learnt_clauses) {
        // Fake assignments map for garbage collection (not used for learned clauses)
...
void ClauseDatabase::initWatches() {
    ...
    for (size_t id = 0; id < clauses.size(); id++) {
        if (!clauses[id]) continue; // Skip deleted clauses
...
void ClauseDatabase::bumpClauseActivity(ClauseID id) {
    ...
    auto& clause = clauses[id];
    if (!clause->is_learned) return; // Only bump learned clauses
...
void ClauseDatabase::garbageCollect(const std::unordered_map<int, bool>& assignments) {
    ...
    for (size_t id = 0; id < clauses.size(); id++) {
        if (!clauses[id]) continue; // Skip already deleted clauses
